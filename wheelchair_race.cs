 ...
 * 	PictureBox [] Pict = new PictureBox[5];
    ...
    */
    void encountObject()
    {
...

        {
            ...
            if (emblem.Right == 0)
            {
                ...
                end.Text = "レコード記録：" + text + "\nあなたの記録：" + points;
                end.Parent = this;
                wheelchair.Parent = null;
                point.Parent = null;
                end.Height = 1000;
            }
    ...
    void Record()
    {
        StreamReader sr = new StreamReader(@"Z:\パソコン部\文化祭発表用フォルダ\exes\tmp.txt", Encoding.GetEncoding("Shift_JIS"));
        ...
        if (points > num)
        {
            StreamWriter writer = new StreamWriter(@"Z:\パソコン部\文化祭発表用フォルダ\exes\tmp.txt",false, Encoding.GetEncoding("Shift_JIS"));
